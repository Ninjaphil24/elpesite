...
<?php
    ...
    if (isset($_POST['insert'])) {
        ...
        if (count($errors) == 0) {
            ...
            $query = "INSERT INTO bcomments (userID, entryID, comment, commentCreatedOn) VALUES ('" . $fetch_info['id'] . "','$eid','$comment',NOW())";

            $result = mysqli_query($con, $query);

            if ($result) {
                header("Location: barticle.php?eid=$eid&reviewtype=$reviewtype&title=$title#comment");
                die();
...
<header>
      ...
      <nav>
        <ul>
          <li><a href="boardmembers.php">ΑΡΧΙΚΗ</a></li>
          ...
          <div class="pdf">
           ...
           <div class='delete deleteentry'>
            <p>Για 30 λεπτά μπορείτε να σβήσετε αυτή την ανάρτηση</p>
              <a href='bdeleteentry.php?eid=<?php echo $eid ?>' class='confirmation' onclick="return confirm('Are you sure?');">Delete Entry</a>
            </div>
   </div>
   <?php }
           }
...
LEFT JOIN bentries_likes
ON bentry.eid = bentries_likes.article

LEFT JOIN usertable
ON bentries_likes.user = usertable.id
...
LEFT JOIN bentries_dislikes
ON bentry.eid = bentries_dislikes.article

LEFT JOIN usertable
ON bentries_dislikes.user = usertable.id
...
?>>
  <?php foreach ($articles as $article): ?>
    <div class="yesVote">
      <a href="blike.php?type=article&id=<?php echo $eid; ?>&userid=<?php echo $fetch_info['id']; ?>&reviewtype=<?php echo $reviewtype; ?>&title=<?php echo $title; ?>"
    ...
    <div class="noVote">
      <a href="bdislike.php?type=article&id=<?php echo $eid; ?>&userid=<?php echo $fetch_info['id']; ?>&reviewtype=<?php echo $reviewtype; ?>&title=<?php echo $title; ?>" onclick="return confirm('Πρόκειται να ψηφίσετε ΟΧΙ.  Αν πατήσετε ΟΚ η ψήφος θα καταχωρηθεί και δεν θα μπορείτε να αλλάξετε την ψήφο σας.');">OXI</a> <br>
...
<div class="commentsout">
       <?php
           ...
           $sql = "SELECT * FROM bcomments INNER JOIN usertable ON bcomments.userID = usertable.id INNER JOIN bentry ON bcomments.entryID = bentry.eid WHERE entryID = '$eid' ORDER BY bcomments.cid";
        ...
        if ($fetch_info['id'] == $row['userID'] && time() - strtotime($row['commentCreatedOn']) < 1800) { ?>
            ...
            <div class="buttons">
              <form class='delete' method='POST' action='barticle.php?eid=<?php echo $eid ?>&reviewtype=<?php echo $reviewtype ?>&title=<?php echo $title ?>#comment'>
              ...
              <div class='delete'>
              <a href='bdeletecomment.php?cid=<?php echo $row['cid'] ?>&eid=<?php echo $eid ?>&reviewtype=<?php echo $reviewtype ?>&title=<?php echo $title ?>' class='confirmation' onclick="return confirm('Are you sure?');">Delete Comment</a>
              </div><br>
            </div><?php ;} else {
                                  echo "";
                              }
   ...
   <?php
    ...
    <form method='post' action='bedit.php?eid=$eid&reviewtype=$reviewtype&title=$title&cid=$cid&comment=$comment'>
    <input type='hidden' name='cid' value='" . $cid . "'>
    <textarea placeholder='Write a comment.' type='text' name='comment' id='comment'>" . $comment . "</textarea>
