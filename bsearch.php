...
if (isset($_POST['insert'])) {
    ...
    if (count($errors)==0) {
      $query = "INSERT INTO bentry(userIDentry,reviewtype,title,link,biog,createdOn)
  ...
  die();
      ...
      $query = "SELECT * FROM bentry WHERE reviewtype = '$reviewtype' AND title = '$title'";
      ...
      header("Location: barticle.php?eid=".$row['eid']."&reviewtype=".$row['reviewtype']."&title=".$row['title']."");
     die();  }
    }
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="form-group">
                  ...
                  <label style="padding:2px;">&nbsp;&nbsp;ΤΙΤΛΟΣ ΑΝΑΡΤΗΣΗΣ</label>
  ...
  <div class="search-user">
    <form action="search.php" method="POST"> 
      ...
      <div class="categoryButtons">
        <button type="submit" name="submit-search">&#8593;  ΕΠΙΛΟΓΗ ΚΑΤΗΓΟΡΙΑΣ  &#8593;</button>
        <a href="boardmembers.php">ΟΛΕΣ ΟΙ ΚΑΤΗΓΟΡΙΕΣ</a>
      </div>
    </form>
  </div>
<!-- Create a "by category" button that will connect with the search script. -->
  ...
  <?php if($fetch_info['usertype']=='admin'){
    echo '
  <a href="home.php">ΧΩΡΟΣ ΜΕΛΩΝ</a>
  <a href="boardmembers.php" class="active">ΧΩΡΟΣ ΔΣ</a>';}?>
  </div> -->
...
<ul>
  ...
  <?php
    ...
    if(isset($_POST['submit-search'])) {
        ...
        $sql = "SELECT * FROM bentry WHERE reviewtype LIKE '%$search%' ORDER BY eid DESC LIMIT $offset, $total_records_per_page";
        ...
        if($queryResult > 0) {
            while ($row = mysqli_fetch_assoc($result)){
                ...
                echo "<a href='barticle.php?eid=".$row['eid']."&reviewtype=".$row['reviewtype']."&title=".$row['title']."'>

                <li><p><span>".$row['reviewtype']."<br> 
                <h5>".$row['title']."</h5><br>
